...
start() {
    echo "Predictor Pro FINAL | Motor de Equilíbrio Oráculo | Sincronia SP"
...
async function run() {
    ...
    // Sincronia Oráculo (Âncora de Realidade)
    setInterval(async () => {
        try {
    ...
    ws.on("message", (data) => {
        ...
        if (stream === "btcusdt@aggTrade") {
            ...
            // --- MOTOR DE PROBABILIDADE HÍBRIDO (Ciclo 5m) ---
            const elapsed = (now.getMinutes() % 5) * 60 + now.getSeconds();
            const progress = elapsed / 300;
            const secToWindow = 300 - elapsed;

            // Pesos Dinâmicos baseados no Oráculo vs Binance
            ...
            // Logica do Oráculo: Se Binance < CL, score alto para ALTA (atração)
            const oracleGap = chainlinkPrice - currentPrice;
            const scoreOracle = oracleGap > 0 ? 0.8 : (oracleGap < 0 ? 0.2 : 0.5);

            // Mixagem final (Inércia ganha peso conforme o fim do ciclo chega)
            ...
            const clColor = chainlinkPrice >= (lastChainlinkPrice || chainlinkPrice) ? "\x1b[32m" : "\x1b[31m";
            ...
            // OUTPUT
            process.stdout.write(`\r\x1b[K[${now.toLocaleTimeString("pt-BR")}] Ini: $${priceAtWindowStart.toFixed(2)} | BINANCE: $${currentPrice.toFixed(2)}\x1b[0m | Janela: ${winColor}$${diffWin.toFixed(2)} (${(pctWin>=0?"+":"")}${pctWin.toFixed(3)}%)\x1b[0m | CL: ${clColor}$${chainlinkPrice.toFixed(2)}\x1b[0m\n`);
